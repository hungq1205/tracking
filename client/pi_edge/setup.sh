...
echo "--- apt packages (python3-picamera2, python3-venv, libportaudio2) ---"
sudo apt update
# libportaudio2 is the native shared library the 'sounddevice' pip package
# binds to -- pip alone does not provide it on ARM, and importing
# sounddevice fails with "OSError: PortAudio library not found" without it.
