...
class AndroidMockClient(IClientApp):
    def run(self, video_source: str, prompt: str, server_ip: str, stream_to_server: bool):
        ...
        # For a real Android app, this would be replaced by actual camera access (e.g., Android CameraX)
        emulator = CameraEmulator(video_source, target_fps=15) # Targeting 15 FPS for stability
        
        print(f"[ANDROID MOCK] Connecting to Server at {server_addr}...")
        last_gui_send = 0
        last_loop_time = time.perf_counter()
        initialized = False

        cv2.namedWindow("Android Mock View", cv2.WINDOW_NORMAL)
        ...
        for frame, _ in emulator.stream():
            ...
            # Push to Server GUI if enabled (Throttled to 5 FPS)
            if stream_to_server:
                now = time.time()
                if (now - last_gui_send) > 0.2:
                    remote_detector.send_gui_frame(frame)
                    last_gui_send = now

            if not initialized:
                print(f"[ANDROID MOCK] Initializing target: {prompt}")
                ...
                if obj_track.visible:
                    ...
                    # Display initial frame even if detection fails
                    cv2.imshow("Android Mock View", frame)
            ...
            cv2.imshow("Android Mock View", vis_frame)
